...
class Messenger(object):
  ...
  def send(self, commit_id, author, message):
    self.server.bot_commit.recordCommit(commit_id, author, message)
...
def process_commits(commits, messenger):
    for ref_name in commits.keys():
        ...
        for i, commit in enumerate(commits[ref_name]):
            ...
            if use_index:
                commit_id = '%s commit (#%d) %s' % (ref_name, i + 1, 
                                                       commit_hash)
            else:
            ...
            messenger.send(commit_id, author, message)
